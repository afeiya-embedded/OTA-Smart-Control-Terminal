...
void WriteCmd(unsigned char I2C_Command)//写命令
{
	...
	buf[0] = 0x00;
	buf[1] = I2C_Command;
	// I2C_WriteByte(I2C1,OLED_ADDRESS,buf, 2);   替换这个函数 
	// OLED_ADDRESS 器件地址 
	// buf 发送的数据地址 
	// 2 :发送的字节数 
	// 1000 : 超时时间 
...
{	
	...
	buf[0] = 0x40;
	buf[1] = I2C_Data;
	//I2C_WriteByte(I2C1,OLED_ADDRESS,buf, 2);
	HAL_I2C_Master_Transmit(&hi2c1, OLED_ADDRESS, buf, 2, 1000); 
}

void OLED_Init(void)
{
	HAL_Delay(1); //这里的延时很重要 , 使用HAL库替换  , 延时至少是100us 
...
// Calls          : 
// Parameters     : x,y -- 起始点坐标(x:0~127, y:0~7); N:汉字在codetab.h中的F16x16[]的下标
// Description    : 显示codetab.h中的汉字,16*16点阵
// 这个屏幕最多可以显示4行汉字  , 每一行可以 显示8个汉字  , 这个屏幕最多可以显示32个汉字  
//--------------------------------------------------------------
...
void OLED_Test(void)
{
		...
		OLED_Fill(0x00);//全屏灭
		HAL_Delay(2000);
		for(int i=0;i<4;i++)// 显示几个汉字
		{
			OLED_ShowCN(22+i*16,0,i+7);//测试显示中文
		}
		HAL_Delay(2000);
		OLED_ShowStr(0,3,(unsigned char*)"OLED Test",1);          //测试6*8字符
